  ...
  try {
    ...
    for (const user of users) {
      try {
        ...
        if (err.code === 'ER_DUP_ENTRY') {
          console.log(`중복된 유저 '${user.username}' 건너뜀`);
        } else {
          throw err;
        }
      }
    ...
    for (const review of reviews) {
      await db.query(
        ...
        [review.movie_id, review.user_id, review.rating, review.content, '1234']
      );
    }
    ...
    for (const comment of comments) {
      await db.query(
        ...
        [comment.review_id, comment.user_id, comment.content, '1234']
      );
    }

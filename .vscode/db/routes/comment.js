...
// 비밀번호 확인 및 폼 렌더링 함수 (댓글/리뷰 공통 사용)
async function checkPasswordAndRenderForm(req, res, itemId, itemType, renderView) {
    ...
    try {
        ...
        if (!passwordMatch) {
            // 비밀번호 불일치 시 폼을 다시 렌더링하고 오류 메시지 전달
            let item;
            if (itemType === 'comment') {
                ...
                item = commentRows[0];
                 ...
                 // 리뷰의 경우, 필요한 데이터를 다시 가져오는 로직을 추가
...
router.get('/delete/:comment_id', async (req, res) => {
    ...
    try {
        ...
        res.render('delete_comment', { comment: comment, error: null });
    } catch (err) {
...
router.post('/edit/:id', async (req, res) => {
  ...
  try {
    ...
    const passwordMatch = await bcrypt.compare(password, rows[0].password);
    if (!passwordMatch) {
      return res.render('edit_comment', { comment: { comment_id: commentId, review_id: review_id, username: username, content: content }, error: '비밀번호가 일치하지 않습니다.' });
    }
